...
#SBATCH --time=0-00:05
# job stdout file
#SBATCH -o OUT/mutate_bgn.%J
# job stderr file
#SBATCH -e ERR/mutate_bgn.%J
# job name id
#SBATCH --job-name=mutate_bgn
###
...
# working directory
working=$(pwd)
# location for pdb file, residue types and positions files
tmp=$working/../tmp
...
resPos=( $(<"$resources/benign_res_pos.txt") ) # reisdue position number
resTy=( $(<"$resources/benign_res_ty.txt") ) # residue type

######################################################################

# copy model and script to modeller-10.4 directory
cp $tmp/$model.pdb $modeller/modeller-10.4
...
for ((i=0; i<${#resPos[@]}; i++)); do
    mv $model${resTy[i]}${resPos[i]}.pdb $output/mutated_pdb_files/benign
    mv $model${resTy[i]}${resPos[i]}.log $tmp
done

# remove model and mutate_model_2chains.sh copy from modeller-10.4 directory
rm $model.pdb $script
...
echo "Submitting foldx_stability_slurm.sh to SLURM"
echo
# submit foldx_stability_slurm.sh to calculate Delta G of fold stability
sbatch foldx_stability_slurm.sh