...
# gene symbol name
gene=FKRP
# full directory pathway to output - CHANGE PATHWAY UNTIL PROJECT
...
# create array variables to iterate through in for loop
resPos=( $(<"$tmp/benign_res_pos.txt") ) # reisdue position number
resTy=( $(<"$tmp/benign_res_ty.txt") ) # residue type
...
# submit foldx_stability.sh to conduct thermodynamic analysis of repaired wild type and mutant pdbs
sbatch foldx_stability.sh